...
# -*- coding: utf-8 -*-
"""
Test OpenAI Native Entity Extractor
"""
...
from src.extract.openai_native_entity_extractor import create_extractor


def test_openai_native_extractor():
    """Test OpenAI native entity extractor"""

    print("=" * 60)
    print("OpenAI Native Entity Extractor Test")
    print("=" * 60)
...
韩立从未想到，此次出去后钱财的多少对他已失去了意义，他竟然走上了一条与凡人不同的仙业大道，走出了自己的修仙之路。
    ...
    test_chunk = ChapterChunk.create_chunk(
        ...
        line_end=20,
        pos_start=0,
        pos_end=len(test_text.strip()),
        token_count=len(test_text.strip()) // 4
    )

    print(f"Test text length: {len(test_text)} characters")
    print(f"Estimated tokens: {len(test_text) // 4}")
    print()
    ...
    try:
        ...
        result = extractor.extract_entities_and_relations(test_chunk)
        ...
        print(f"   Direct relationships: {len(result.relationships)}")
        print(f"   Other relationships: {len(result.other_relationships)}")
        ...
        if result.other_relationships:
            print(f"🔗 Other Relationships ({len(result.other_relationships)}):")
            for i, relation in enumerate(result.other_relationships, 1):
                print(f"   {i}. {relation.source} → {relation.target}")
                print(f"      Description: {relation.description}")
                print()

        if not result.entities and not result.relationships:
            print("⚠️  No entities or relationships were extracted.")
            print("   This might indicate an issue with the API configuration or response format.")
